...
void ULCturnRight(int p_fltxyRadius0, int p_fltxy0,int p_fltxyRadius1)
{
	if(p_fltxyRadius0 >= p_fltxy0)
				{
					cout<<"After Turn Right";
					cout<<"Move Forward";
					delay('F');
...
int main()
{
    ...
    while(1)
    {
       ...
       // cvShowImage("Processed Blue",p_imgProcessed_v1);
		...
		if(dest!=0 && src!=0)
		{
			if(p_fltxyRadius[1] < p_fltxy[1] && p_fltxyRadius[0] < p_fltxy[0] && ULCFlag == false && URCFlag == false && LRCFlag == false)
			{
				URCFlag = true;
				cout<<"URC On";
			}else if(p_fltxyRadius[1] < p_fltxy[1] && p_fltxyRadius[0] > p_fltxy[0] && ULCFlag == false && URCFlag == false && LRCFlag == false)
			{
				ULCFlag = true;
				cout<<"ULC On";
			}else if(p_fltxyRadius[1] > p_fltxy[1] && URCFlag == false && LRCFlag == false && ULCFlag == false)
			{
				LRCFlag = true;
			}
			if(URCFlag == true)
			...
			if(ULCFlag == true)
			{
				...
				}else if((p_fltxyRadius[1] >= (p_fltxy[1]-120)) && p_fltxyRadius[1] > 0)
				{
					if(p_fltxyRadius[1] <= p_fltxy[1] && p_fltxyRadius[1] > 0)
					{
						...
						cout<<"Turn Right";
						delay('R');
						ULCturnRight(p_fltxyRadius[0],p_fltxy[0],p_fltxyRadius[1]);
					}else{
							//delay('S');
					}
				}
